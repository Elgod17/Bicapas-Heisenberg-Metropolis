'ESTE CODIGO SIMULA LA BICAPA FERROMAGNETICO/PARAMAGNETICO'
'ESTE CÓDIGO ES LA VERSION OPTIMIZADA Y CORREGIDA POR GEMINI 3 DEL CODIGO "bicapa.py"'
...
@njit
def metropolis(config, T, pasos_term, pasos_med, l, k, h, J, J_interaccion):
    cfg = config
...
def m_vs_t(Ts, p_term, p_med, l, k, h, J, J_interaccion):
    arr_mz_p, arr_mod_p = [], []
    arr_mz_f, arr_mod_f = [], []
    arr_c = []
    ...
    for T in Ts:
        ...
        mz_p, mod_p, mz_f, mod_f, c = metropolis(curr_config, T, p_term, p_med, l, k, h, J, J_interaccion)
...
def simulaciones_mt(Tmax, Tmin, deltaT, p_term, p_med, max_workers, num_replicas, l, k, h, J, J_interaccion):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, mp_context=ctx) as executor:
        ...
        futures = [executor.submit(m_vs_t, Ts, p_term, p_med, l, k, h, J, J_interaccion) for _ in range(num_replicas)]
...
if __name__ == "__main__":
    ...
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(f"Calor_Especifico_{pasos_medicion}medidas.png", dpi=200)
    plt.close()
